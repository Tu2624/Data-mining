      ...
      <div className="flex flex-col items-center justify-center py-40 gap-6">
        ...
        <div className="text-center space-y-2">
          <p className="font-black text-slate-900 uppercase tracking-widest text-sm">
            Hồ sơ cá nhân chưa sẵn sàng
          </p>
          <p className="text-slate-400 font-medium italic text-xs">
            Vui lòng kết nối thuật toán để tiếp tục.
          </p>
        </div>
      </div>
    );
        ...
        <div className="absolute -bottom-24 left-8 right-8 flex flex-col md:flex-row items-end gap-8 md:gap-12">
          ...
          <div className="flex-1 pb-6 flex flex-col md:flex-row items-center md:items-end justify-between gap-8 w-full">
            <div className="text-center md:text-left space-y-3">
              ...
              >
                <span className="flex items-center gap-2 hover:text-indigo-500 transition-colors">
      ...
      <div className="grid grid-cols-3 gap-6 md:gap-10 mb-16 sm:px-8">
        {[
          ...
          },
          {
            ...
            hover: "group-hover:border-rose-100",
          },
          {
            label: "Following",
            ...
            hover: "group-hover:border-emerald-100",
          },
        ].map((stat, i) => (
          <motion.div
            key={stat.label}
          ...
          >
            <div
              className={`p-4 rounded-[22px] ${stat.color} mb-2 group-hover:scale-110 transition-transform shadow-sm`}
            >
              <stat.icon size={24} />
            </div>
            ...
            >
              ...
              {activeTab === tab.id && (
                <motion.div
                  ...
                  className="absolute bottom-[-2px] left-0 right-0 h-1 bg-indigo-600 rounded-full"
                />
              )}
            </button>
          ))}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                {currentData.length > 0 ? (
                  currentData.map((post, idx) => (
                    ...
                    >
                      <PostCard post={post} onUpdate={fetchProfileData} />
                    </motion.div>
                  ))
                ) : (
                    ...
                    (u, idx) => (
                      ...
                      >
                        ...
                        <div className="text-center space-y-1">
                          <h4 className="font-black text-slate-900 text-lg tracking-tight uppercase truncate max-w-[120px]">
                            {u.username}
                          </h4>
                        ...
                        <Link
                          to={`/profile/${u.id}`}
                          className="mt-2 px-8 py-2.5 bg-slate-50 text-slate-400 group-hover:bg-indigo-600 group-hover:text-white rounded-[18px] font-black text-[10px] uppercase tracking-widest transition-all"
                        >
                          Xem hồ sơ
                        </Link>
                      </motion.div>
...
const EmptyState = ({ tab }) => (
  <div className="col-span-full py-32 bg-slate-50/50 border-4 border-dashed border-slate-100 rounded-[56px] flex flex-col items-center justify-center text-center space-y-8 w-full">
    <div className="p-10 bg-white rounded-full shadow-soft text-slate-200 border border-slate-100">
      {tab === "favorites" ? (
        <Heart size={64} strokeWidth={1} className="text-rose-200" />
      ) : (
        <Grid size={64} strokeWidth={1} className="text-indigo-200" />
      )}
    </div>
    <div className="space-y-2">
      ...
      <p className="text-slate-400 text-sm font-medium italic max-w-sm">
        {tab === "followers"
          ? "Hãy chia sẻ thêm trải nghiệm thú vị để thu hút những người cùng tần số nhé!"
          : "Hãy tìm kiếm những người bạn mới để cùng thảo luận về ẩm thực AI!"}
      </p>
    </div>
    <Link
      to="/"
      className="px-12 py-4 bg-indigo-600 text-white rounded-[24px] font-black text-[11px] uppercase tracking-widest hover:shadow-xl hover:shadow-indigo-600/20 transition-all active:scale-95"
    >
      Khám phá Feed AI
    </Link>
  </div>
