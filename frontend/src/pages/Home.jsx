...
const Home = () => {
    ...
    const getFilteredPosts = () => {
        ...
        if (activeFilter === 'Thịnh hành') {
            ...
            result.sort((a, b) => (b.rating_count || 0) - (a.rating_count || 0) + (Math.sin(a.id) - Math.sin(b.id)));
        }
        return result;
    };
        ...
        <div className="max-w-7xl mx-auto pb-20 px-4 md:px-0">
            ...
            >
                ...
                <div className="relative z-10 max-w-3xl space-y-8">
                    ...
                    <p className="text-slate-400 text-xl font-medium max-w-xl leading-relaxed">
                        Dựa trên hàng triệu tương tác cộng đồng, FoodRec AI kiến tạo hành trình ẩm thực cá nhân hóa dành riêng cho bạn.
                    </p>
            ...
            {loading ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {[1, 2, 3, 4, 5, 6].map((i) => (
                        <div key={i} className="bg-slate-100 rounded-[40px] aspect-[4/5] animate-pulse"></div>
                    ))}
                </div>
            ) : posts.length === 0 ? (
                <div className="py-40 text-center space-y-6">
                    ...
                    <div className="space-y-2">
                        <p className="text-xl font-black text-slate-900">Không tìm thấy món ăn nào!</p>
                        <p className="text-slate-400 font-medium italic">Hãy thử từ khóa khác hoặc quay lại sau.</p>
                    </div>
                </div>
            ) : (
                <motion.div 
                ...
                >
                    {displayPosts.map((post) => (
                        <PostCard key={`${activeFilter}-${post.id}`} post={post} onUpdate={fetchPosts} />
                    ))}
                </motion.div>
            )}
        </div>
