...
const Friends = () => {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const endpoint = activeTab === 'following' ? '/social/following' : '/social/suggestions';
    ...
    if (!user) {
        return (
            <div className="text-center py-20 bg-white rounded-3xl shadow-soft">
                ...
                <h2 className="text-2xl font-black text-gray-900">Vui lòng đăng nhập</h2>
                <p className="text-gray-500 mt-2">Bạn cần đăng nhập để xem danh sách bạn bè.</p>
                <Link to="/login" className="mt-6 inline-block bg-primary-500 text-white px-8 py-3 rounded-2xl font-black uppercase tracking-widest text-sm shadow-lg shadow-primary-100 no-underline">Đăng nhập ngay</Link>
            </div>
        );
    }
        ...
        <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="bg-white rounded-[32px] p-8 shadow-soft border border-gray-100 flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    ...
                    <p className="text-gray-500 font-medium mt-1">Xây dựng cộng đồng ẩm thực của riêng bạn</p>
                </div>
            ...
            {loading ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {[1, 2, 4, 5].map(n => (
                        <div key={n} className="bg-white h-24 rounded-3xl animate-pulse shadow-sm border border-gray-50"></div>
                    ))}
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {users.length > 0 ? (
                        users.map(u => (
                            <div key={u.id} className="bg-white rounded-[28px] p-5 shadow-soft border border-gray-50 flex items-center justify-between group hover:border-primary-100 transition-all">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-0.5">Thành viên mới</p>
                                    </div>
                                </div>
                        ...
                        <div className="col-span-full text-center py-20 bg-gray-50/50 rounded-[32px] border-2 border-dashed border-gray-200">
                            {activeTab === 'following' ? (
                                <>
                                    ...
                                    <h3 className="text-xl font-black text-gray-400">Bạn chưa theo dõi ai</h3>
                                    <p className="text-gray-400 mt-1">Hãy chuyển sang mục Khám phá để tìm bạn mới!</p>
                                </>
                            ) : (
                                <>
                                    <Search size={48} className="mx-auto text-gray-200 mb-4" />
                                    <h3 className="text-xl font-black text-gray-400">Không tìm thấy gợi ý</h3>
                                    <p className="text-gray-400 mt-1">Bạn đã theo dõi tất cả mọi người rồi đấy!</p>
                                </>
                            )}
                        </div>
                    )}
