...
import { Heart, MessageCircle, Share2, MoreHorizontal, MapPin, Star, Sparkles } from 'lucide-react';
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center gap-4">
          <Link to={`/profile/${post.user_id}`} className="relative group/avatar">
            <div className="w-12 h-12 rounded-2xl bg-indigo-50 flex items-center justify-center text-indigo-600 font-black border-2 border-white shadow-soft overflow-hidden transition-transform group-hover/avatar:scale-110">
              {post.avatar_url ? (
                ...
                <span className="text-lg font-black">{post.username?.charAt(0).toUpperCase()}</span>
              )}
            </div>
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              <Link to={`/profile/${post.user_id}`} className="font-extrabold text-slate-900 hover:text-indigo-600 transition-all text-[15px] tracking-tight decoration-none">
                {post.username}
              </Link>
            ...
            <div className="text-[10px] text-slate-400 font-black uppercase tracking-widest mt-1 flex items-center gap-2">
               <span className="opacity-70">{post.created_at && !isNaN(new Date(post.created_at)) ? new Date(post.created_at).toLocaleDateString('vi-VN') : 'Đang cập nhật'}</span>
               <span className="w-1 h-1 rounded-full bg-slate-200"></span>
               <span className="flex items-center gap-1 text-indigo-400 group-hover:text-indigo-500 transition-colors"><MapPin size={10} /> {post.location}</span>
            </div>
          </div>
        </div>
      ...
      <div className="px-8 pb-6 space-y-3">
        {post.title && <h3 className="font-black text-slate-900 text-2xl md:text-3xl tracking-tighter leading-tight group-hover:text-indigo-600 transition-colors">{post.title}</h3>}
      ...
      {/* Hình ảnh - Media */}
      <Link to={`/post/${post.id}`} className="block relative aspect-[16/10] bg-slate-50 overflow-hidden mx-4 mb-4 rounded-[32px] group/media">
        <img 
          src={post.image_url} 
          alt={post.title} 
        ...
        <div className="absolute top-6 right-6 glass-premium px-4 py-2 rounded-[20px] flex items-center gap-2 shadow-xl border border-white/40">
          ...
          <span className="text-sm font-black text-slate-900">{Number(post.avg_rating || 0).toFixed(1)}</span>
        </div>
      ...
      <div className="px-6 py-5 bg-white flex items-center justify-between border-t border-slate-50">
        ...
        >
          <Share2 size={18} />
          <span className="hidden sm:inline">{post.share_count > 0 ? post.share_count : 'Chia sẻ'}</span>
        </motion.button>
      </div>
    </motion.div>
  );
