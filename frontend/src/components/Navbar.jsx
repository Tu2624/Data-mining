...
const Navbar = () => {
  ...
  return (
    ...
    >
      {/* Brand */}
      <Link to="/" className="flex items-center gap-3 text-indigo-600 font-extrabold text-2xl tracking-tighter group transition-transform active:scale-95">
        <motion.div 
        ...
        <div className="flex flex-col leading-tight">
          ...
          <span className="text-indigo-500 text-[10px] font-black tracking-[0.4em] uppercase">REC-AI</span>
        </div>
      </Link>
        ...
        <div className="flex items-center gap-5 border-l border-slate-100 pl-8">
              ...
              {showNotifications && (
                ...
                >
                  <div className="flex items-center justify-between px-4 py-3 bg-slate-50/50 rounded-[20px] mb-2">
                    <span className="font-black text-xs uppercase tracking-widest text-slate-900">Thông báo</span>
                    {unreadCount > 0 && (
                      <button
                  ...
                  <div className="max-h-80 overflow-y-auto px-1 space-y-1">
                    {notifications.length === 0 ? (
                      ...
                      notifications.slice(0, 10).map(n => (
                        ...
                        >
                          <p className="font-bold text-slate-800 text-[13px] leading-snug">{n.content}</p>
          ...
          {user ? (
            <div className="flex items-center gap-4">
              <Link to="/profile" className="flex items-center gap-3 p-1.5 pr-4 rounded-2xl hover:bg-white hover:shadow-soft transition group">
                <div className="w-9 h-9 rounded-xl bg-indigo-50 flex items-center justify-center text-indigo-600 overflow-hidden ring-2 ring-transparent group-hover:ring-indigo-100 transition shadow-inner">
                  {user.avatar_url ? <img src={user.avatar_url} alt={user.username} referrerPolicy="no-referrer" className="w-full h-full object-cover" /> : <User size={18} />}
                </div>
                <div className="flex flex-col hidden sm:flex">
                  <span className="font-black text-[13px] text-slate-900 leading-none">{user.username}</span>
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Hội viên DeepMind</span>
                </div>
              </Link>
              <button 
                onClick={logout} 
            ...
            <Link to="/login" className="bg-indigo-600 text-white px-8 py-3 rounded-2xl font-black text-xs uppercase tracking-widest hover:shadow-lg hover:shadow-indigo-500/20 transition active:scale-95">
              Kết nối thuật toán
            </Link>
          )}
        </div>
