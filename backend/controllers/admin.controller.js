...
class AdminController {
  static async createPost(req, res) {
    const { title, description, price, location, categoryId, imageUrl, tags } =
      req.body;
    ...
    try {
      ...
      if (imageUrl) {
        await pool.query("INSERT INTO media (post_id, url) VALUES (?, ?)", [
          postId,
          imageUrl,
        ]);
      }
      ...
      if (tags && tags.length) {
        const tagQueries = tags.map((tagId) =>
          pool.query("INSERT INTO post_tags (post_id, tag_id) VALUES (?, ?)", [
            postId,
            tagId,
          ]),
        );
